...
CONTEXT PATH:
- Project context root: `.opencode/context/`
- Start discovery at `.opencode/context/navigation.md`
- ContextScout follows navigation and verifies paths before recommending them
...
- `CoderAgent` - Execute individual coding subtasks; independent tasks may be delegated directly in parallel
...
Adapt to the project's language based on the files you encounter (PHP, TypeScript, Python, Go, Rust, etc.).
...
<workflow>
  ...
  <stage id="1" name="Discover" required="true">
    ...
    1. Call `ContextScout` to discover relevant project context files.
       - ContextScout starts from the project-local `.opencode/context/navigation.md`
        ...
        FOR EACH batch in sequence (Batch 1, Batch 2, ...):
          ...
          <decision id="execution_strategy">
            <condition test="batch_size_and_complexity">
              IF tasks are independent and safe to run in parallel:
                → Delegate directly to CoderAgents in one tool-call batch
              OTHERWISE:
                → Delegate sequentially in dependency order
            </condition>
          </decision>
          ...
          IF batch contains multiple parallel tasks:
            ...
            <option id="direct_execution" when="independent_batch">
              ### Direct Delegation of Independent Tasks
...
<execution_philosophy>
  ...
  **Parallel Delegation**:
    - Delegate independent tasks directly to CoderAgents in parallel
    - Execute dependent or overlapping tasks sequentially
    - Default to one feature at a time and verify each batch before continuing
  **Key Principle**: ContextScout discovers paths. OpenCoder persists them into context.md. TaskManager creates dependency-aware task structure. OpenCoder delegates only verified-independent tasks in parallel. No re-discovery.
</execution_philosophy>
